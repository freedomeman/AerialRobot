...
void Gimbal::init()
{
    ...
    //初始化云台朝向状态机
    gimbal_turn_flag = GIMBAL_AHEAD; 
    last_gimbal_turn_flag = GIMBAL_AHEAD;
    /*---------------------------yaw电机---------------------------------*/
    //获取Yaw轴电机数据指针
...
void Gimbal::turn_around_control(fp32 *yaw)
{
    ...
    {
        if (gimbal_turn_flag == GIMBAL_AHEAD || gimbal_turn_flag == GIMBAL_REAR)
        {
            last_gimbal_turn_flag = gimbal_turn_flag;
            gimbal_turn_flag = GIMBAL_TURN;
            //保存掉头的目标值
            gimbal_end_angle = rad_format(gimbal_yaw_motor.gyro_angle + PI);
            if(last_gimbal_turn_flag == GIMBAL_AHEAD)
            {
                gimbal_yaw_motor.offset_ecd = ECD_YAW_REAR_MID;
            }else
            {
                gimbal_yaw_motor.offset_ecd = ECD_YAW_MID;
            }
        }
    }
    ...
    //云台属于旋转中
    if (gimbal_turn_flag == GIMBAL_TURN)
    {
        gimbal_yaw_motor.gyro_angle_set = gimbal_end_angle;
    }
    //到达pi （180°）后停止存在误差值
    if (gimbal_turn_flag == GIMBAL_TURN && fabs(rad_format(gimbal_end_angle - gimbal_yaw_motor.gyro_angle)) < 0.05f)
    {
        if(last_gimbal_turn_flag == GIMBAL_AHEAD)
        {
            gimbal_turn_flag = GIMBAL_REAR;
        }else
        {
            gimbal_turn_flag = GIMBAL_AHEAD;
        }
...
#endif
#if PITCH_TURN
    gimbal_pitch_motor.current_give = -(int16_t)(gimbal_pitch_motor.current_set + COMPENSATE_SET);
#else
